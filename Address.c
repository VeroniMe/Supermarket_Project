...
{
	BYTE byteAddress[1] = { 0 };
	byteAddress[0] = pAdd->num;

	if (fwrite(&byteAddress, sizeof(BYTE), 1, fp) != 1)
	{
		return 0;
	}
...
int readAddressFromCompressFile(Address* pAdd, FILE* fp)
{
	...
	if (fread(&byteAddress, sizeof(BYTE), 1, fp) != 1)
	{
		return 0;
	}
	pAdd->num = byteAddress[0];



	pAdd->street = readStringFromFile(fp, "error write street name\n");
    CHECK_RETURN_0(pAdd->street);


	pAdd->city = readStringFromFile(fp, "error write city name\n");
	CHECK_RETURN_0(pAdd->city);
	return 1;
}
